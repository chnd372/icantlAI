...
export const listTranslations = query({
  ...
  handler: async (ctx) => {
    ...
    return translations.map((t) => ({
      ...
      model: t.model,
      sourceLang: t.sourceLang ?? "english",
      targetLang: t.targetLang ?? "indonesian",
      status: t.status,
      error: t.error ?? null,
  ...
  handler: async (ctx, args) => {
    ...
    return {
      translation: {
        ...
        sourceLang: translation.sourceLang ?? "english",
        targetLang: translation.targetLang ?? "indonesian",
...
export const importChapter = mutation({
  args: {
    ...
    translatedText: v.string(),
    model: v.string(),
    sourceLang: v.optional(v.union(v.literal("english"), v.literal("chinese"))),
    targetLang: v.optional(v.union(v.literal("english"), v.literal("indonesian"))),
  },
  handler: async (ctx, args) => {
    const userId = await getAuthUserId(ctx);
    if (userId === null) throw new Error("Not signed in");
    ...
    const translationId = await ctx.db.insert("translations", {
      ...
      sourceText: args.sourceText ?? "",
      model: args.model,
      sourceLang: args.sourceLang ?? "english",
      targetLang: args.targetLang ?? "indonesian",
      status: "done",
      segmentCount: 1,
      completedSegments: 1,
      createdAt: now,
